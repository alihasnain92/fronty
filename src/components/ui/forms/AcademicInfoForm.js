...
const AcademicInfoForm = ({ formData = {}, onChange, onValidation }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name.endsWith("ResultStatus")) {
      ...
      if (level === "matric") {
        // Result Status is always "Passed" for Matric, no action needed
      } else {
        if (value !== "Passed") {
    ...
    switch (name) {
      ...
      case "interYear":
        ...
        if (
          formData["matricYear"] &&
          value &&
          parseInt(value) < parseInt(formData["matricYear"]) + 2
        ) {
          fieldErrors[name] =
  ...
  const renderMatriculationSection = () => {
    const level = "matric";
    const ResultStatus = "Passed"; // Always "Passed" for Matric
    return (
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="mt-8 p-4 bg-amber-50 rounded-lg border border-amber-100">
        ...
        <ul className="list-disc list-inside space-y-2 text-sm text-amber-700">
          ...
          <li>
            Roll number is required if your result status is "Passed," "Awaited,"
            or "Supplementary"
          </li>
          <li>
          ...
          <li>
            Intermediate passing year must be at least two years after Matriculation passing year
          </li>
          <li>
          ...
          <li>
            Institute name should match exactly as mentioned in your certificate
          </li>
        </ul>
      </div>
